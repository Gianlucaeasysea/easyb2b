...
const DEFAULTS: HeroContent = {
  badge: "B2B Dealer Platform",
  title_line1: "Your Nautical",
  title_line2: "Business Hub",
  subtitle_line1: "Premium accessories, exclusive B2B pricing",
  subtitle_line2: "& dedicated dealer support",
  description: "Join 250+ dealers worldwide. Exclusive pricing, premium products, and dedicated support to grow your nautical business.",
  stat1_value: "250+", stat1_label: "Dealers",
  stat2_value: "16+", stat2_label: "Products",
  stat3_value: "431+", stat3_label: "Reviews",
};
...
const HeroSection = () => {
  ...
  const h = hero || DEFAULTS;
  const stats = [
    { value: h.stat1_value, label: h.stat1_label },
    { value: h.stat2_value, label: h.stat2_label },
    { value: h.stat3_value, label: h.stat3_label },
  ];

  return (
    <section ref={ref} className="relative min-h-screen flex items-center justify-center overflow-hidden bg-background">
      <div className="absolute inset-0 pointer-events-none">
        <DarkVeil hueShift={0} noiseIntensity={0.03} scanlineIntensity={0} speed={0.4} warpAmount={0.04} resolutionScale={0.75} />
      </div>
      ...
      <motion.div className="relative z-10 container mx-auto px-4 text-center pt-20 pb-16" style={{ y: textY, opacity }}>
        ...
        >
          {h.title_line1}<br /><span className="text-gradient-blue">{h.title_line2}</span>
        </motion.h1>

        <motion.h2 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.2 }}
          className="font-heading text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold mb-10 leading-snug text-muted-foreground"
        >
          {h.subtitle_line1}<br className="hidden sm:block" />
          <span className="text-foreground"> {h.subtitle_line2}</span>
        </motion.h2>
      ...
      <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2.5 }} className="absolute bottom-10 left-1/2 -translate-x-1/2 z-10">
